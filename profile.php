            ...
            if (empty($first_name) || empty($last_name) || empty($address_line1) || empty($city) || empty($postal_code) || empty($province)) {
                ...
                $result = $db->insert(
                    "INSERT INTO addresses (user_id, address_type, first_name, last_name, 
                     address_line1, address_line2, city, state, province, postal_code, country, phone, latitude, longitude, is_default) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [$user_id, $address_type, $first_name, $last_name, $address_line1, 
                     $address_line2, $city, $state, $province, $postal_code, $country, $phone, $latitude, $longitude, $is_default]
                );
        ...
        <form method="POST" id="addressForm">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="modal_state" name="state">
                </div>
...
function updateAddressFields(feature) {
    // Mapbox response format is different from Google Maps
    ...
    // Extract information from Mapbox context
    context.forEach(item => {
        ...
        if (id.includes('place')) {
            ...
            region = item.text;
            province = item.text; // Use region as province
    ...
    // Extract street address from place_name (first part before comma)
    if (placeName) {
    ...
    console.log('Extracted address components:', {
        city, province, postalCode, addressLine1, state, region
    });
    ...
    // Update postal code
    if (postalCode) {
        document.getElementById('modal_postal_code').value = postalCode;
    }
    ...
    if (addressLine1) {
        document.getElementById('modal_address_line1').value = addressLine1;
    }
    
    // Show success message
    showAddressFillMessage('Address information has been automatically filled from the map location.');
}
